...
`AppState` (in `data-plane/src/serve.rs`) carried the S-1-projected
...
- `data-plane/src/cache.rs` — `TorrentFileId` got `tf_id_durable`;
...
- `data-plane/src/serve.rs` — `AppState` got `tf_id_durable`;
...
- `data-plane/src/main.rs` — both `AppState` constructions
  populated `tf_id_durable` from `resp.torrent_file.id`.
- `data-plane/src/manager.rs` — `Slot.tf_id` was `tf.id.clone()`
  (the host PK), not `tf.info_hash.clone()`.
- `data-plane/src/provider.rs` — three `DeliveryCapability::new`
...
- `data-plane/src/cache.rs` — `TorrentFileId` was restructured:
...
- `data-plane/src/serve.rs` — both `TorrentFileId { ... }` literals
...
- `data-plane/src/manager.rs` — `Slot` gained
...
- `data-plane/src/capability.rs` — `DeliveryCapability::torrent_file_id`
...
Added five new in-crate tests in `data-plane/src/cache.rs`. All
...
`data-plane/src/cache.rs`). The key is computed once at
...
**Tests added** (`data-plane/src/cache.rs`):
...
`data-plane/src/cache.rs` contains a focused adversarial test
