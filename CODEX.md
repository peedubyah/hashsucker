...
- NOT IMPLEMENTED: authentication, movies requests, series/season/multi-episode requests, importer HTTP transport, rich importer progress.
- INTENTIONALLY DISABLED: movie request buttons and every request mode other than one explicit TV episode. Search can display movies with a visible unsupported label.
...
- Most recent `npm test`: VERIFIED, 5 test files passed, 0 failed (2026-08-19).
- VERIFIED: intent/handoff season-pack invariant, queue filesystem lifecycle, cache enrichment/unknown state, route/static behavior, request validation, handoff construction, TorBox state through API projection, and secret/raw-data exclusion.
...
- VERIFIED LOCALLY: pure pane-state tests cover processing, done, and failed terminology/action state. Full suite remains 5 test files, 0 failures.
