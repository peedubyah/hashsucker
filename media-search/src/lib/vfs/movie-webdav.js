...
export function createMovieWebDav({
  ...
  return async function handleMovieWebDav(request, response, url) {
    if (url.pathname !== DAV_ROOT && !url.pathname.startsWith(`${DAV_ROOT}/`)) return false;
    ...
    try {
      ...
      if (method === 'PROPFIND') {
        ...
        await sendDavXml(response, entries, async (entry) => entry.type === 'file'
          ? ensureMetadata(entry.state)
