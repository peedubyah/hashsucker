// p14-5-rd-to-tb.mjs - P14 step 5: RD->TB shielding proof.
//
// What this script proves:
//   1. With a dual-provider container (no allowlist), set
//      HY4_FORCE_FAIL_PROVIDER=tf_5de34a78...:realdebrid so the RD slot is
//      surgically removed BEFORE CapabilityManager construction.
//   2. TB is the only remaining slot, and it serves 206 with the same
//      reference SHAs (52daa79d4aff / 977afd3ce097 / 11c81ee706e0).
//   3. This is the mirror of P14-4 (TB->RD shielding).
...
const LABEL = process.env.LABEL || 'p14-5-rd-to-tb';
...
async function run() {
  ...
  if (!allOk) {
    log('ABORT: RD->TB shielding proof FAILED');
    process.exit(2);
  }
  log('DONE: RD->TB shielding proven - TB slot served 206 with reference SHAs');
  process.exit(0);
}
