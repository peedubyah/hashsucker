//! T14 transplant proof: bounded automatic two-lane activation.
//!
//! Proven as HY4 P2R on m3-north-db: a decision seam that chooses between
//! the existing single producer and the existing T11-T13 two-lane
//! scheduler based only on missing-work size and availability of a second
//! already-warm same-TF capability. No new scheduler behavior.
//!
...
//! test-injectable), the T11-T13 scheduler engages only when ALL hold:
//! AUTO enabled, the consecutive missing run has >= min chunks, and the T2
//! `reserve_standby` returns a second warm same-TF cap. The reservation
